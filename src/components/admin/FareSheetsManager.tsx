...
export default function FareSheetsManager() {
  ...
  const handleSave = async () => {
    if (!form.title || !form.file_url) return alert("Title and File URL are required");
    if (editing) {
      await supabase
        .from("fare_sheets")
      ...
      await supabase.from("fare_sheets").insert([form]);
    }
    setShowForm(false);
    setEditing(null);
    setForm({});
  ...
  const handleDelete = async (id: number) => {
    ...
    await supabase.from("fare_sheets").delete().eq("id", id);
    fetchItems();
  };

  const toggleActive = async (id: number, current: boolean) => {
    await supabase.from("fare_sheets").update({ is_active: !current }).eq("id", id);
    fetchItems();
  };
