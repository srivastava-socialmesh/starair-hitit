    ...
    <section className="py-12 px-4 bg-gradient-to-b from-white to-gray-50">
      <div className="max-w-7xl mx-auto">
        <div className="mb-6 text-center">
          <span className="text-rose-500 text-sm font-semibold uppercase tracking-wider">📰 Latest</span>
          <h2 className="text-4xl font-bold text-gray-900">News & <span className="text-rose-500">Updates</span></h2>
          <p className="text-gray-500 mt-1">Stay informed with our latest announcements</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {news.map((item) => (
            <div key={item.id} className="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200 hover:border-rose-300 transition overflow-hidden group shadow-sm">
              {item.image_url && (
              ...
              <div className="p-5">
                ...
                <Link href={`/news/${item.slug}`} className="inline-block mt-3 text-rose-600 hover:text-rose-700 font-medium text-sm">
                  Read More →
                </Link>
              </div>
            </div>
