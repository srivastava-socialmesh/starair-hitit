        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {news.map((item) => (
            <div key={item.id} className="bg-white/80 backdrop-blur-sm rounded-2xl border border-gray-200 hover:border-accent transition overflow-hidden group shadow-sm">
              {item.image_url && <div className="relative h-48 w-full"><img src={item.image_url} alt={item.title} className="w-full h-full object-cover group-hover:scale-105 transition duration-500" /></div>}
              <div className="p-5">
                ...
                <Link href={`/news/${item.slug}`} className="inline-block mt-3 text-accent hover:text-[#b00226] font-medium text-sm">Read More →</Link>
              </div>
            </div>
          ))}
        </div>
        <div className="text-center mt-8">
          <Link href="/news" className="inline-block bg-accent hover:bg-[#b00226] text-white font-semibold py-2 px-6 rounded-full shadow-lg shadow-accent/30 transition">View All News</Link>
        </div>
      </div>
    </section>
  );
