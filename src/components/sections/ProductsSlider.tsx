    ...
    <section className="py-12 px-4 bg-gradient-to-b from-white to-gray-50">
      <div className="max-w-7xl mx-auto">
        <div className="mb-8 text-center">
          <span className="text-rose-500 text-sm font-semibold uppercase tracking-widest">✈️ Fly Smart</span>
          <h2 className="text-4xl font-bold text-gray-900">Our <span className="text-rose-500">Products</span></h2>
          ...
          {products.map((product) => {
            ...
            return (
              <SwiperSlide key={product.id}>
                <div
                  className="group relative rounded-2xl overflow-hidden h-[420px] cursor-pointer shadow-lg bg-white border border-gray-200 hover:border-rose-300 transition duration-300"
                ...
                >
                  ...
                  <div className="absolute bottom-24 left-0 w-full px-6 z-10 transition-all duration-300">
                    ...
                    {product.price && <p className="text-rose-300 text-xl font-bold mt-1">${product.price}</p>}
                    <p className="text-white/80 text-sm mt-1">{product.category || "Economy"}</p>
                  </div>

                  <div
                    className={`absolute bottom-0 left-0 w-full bg-white/95 backdrop-blur-md rounded-t-2xl shadow-2xl z-20 transition-all duration-400 ease-in-out overflow-hidden ${
                  ...
                  >
                    <div className="p-5 pt-8">
                      ...
                      <h4 className="text-lg font-bold text-rose-600 mb-2">{product.name} Features</h4>
                      <ul className="space-y-2 text-sm text-gray-700">
                        {product.features?.length > 0 ? (
                          product.features.map((f, idx) => (
                            <li key={idx} className="flex items-start gap-2">
                              <Check size={16} className="text-rose-500 flex-shrink-0 mt-0.5" />
                              <span>{f}</span>
                            </li>
                          ))
                        ) : (
