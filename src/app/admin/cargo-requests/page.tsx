    ...
    <AdminLayout>
      <div className="p-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl font-bold text-amber-400">Cargo Requests</h1>
            <p className="text-slate-400 text-sm">Manage all cargo booking requests</p>
          </div>
          <div className="text-sm text-slate-400">Total: {requests?.length || 0}</div>
        </div>
        ...
        {error ? (
          ...
          <div className="bg-[#111827] border border-white/10 p-8 rounded-xl text-center text-slate-400">
            No cargo requests received yet.
          </div>
        ) : (
              ...
              <tbody>
                {requests?.map((req) => (
                  <tr key={req.id} className="border-b border-white/5 hover:bg-white/5">
                    ...
                    <td className="p-3 text-slate-300">{req.cargo_type}</td>
                    <td className="p-3 text-slate-300">{req.weight}</td>
